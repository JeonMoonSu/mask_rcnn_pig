...
def display_instances(image, boxes, masks, class_ids, class_names,
    ...
    if cbal:
        ...
        while a<i:
            ...
            print(x2, y2, x3, y3, x3-x2, y3-y2)

            multiTracker.add(createTrackerByName(trackerType),image,(x2,y2,x3-x2,y3-y2))
            
            a=a+1
            
        cbal = False

    success, t_boxes = multiTracker.update(image)
    a=0
    j=i
    test_len=10000
    global ex_box
    for i, newbox in enumerate(t_boxes):
        ...
        while a<j:
            ...
            y4=int(newbox[1])
            y5, x5, y6, x6=boxes[a]
            length=math.pow(x4-x5) + math.pow(y4-y5)
            length=math.sqrt(legnth)
            if length<test_len:
                test_len=length
            if length<new_len:
                new_len=length
       ...
       # p = patches.Rectangle(p1, int(newbox[2]), int(newbox[3]), linewidth=2,alpha=0.7, linestyle="dashed",edgecolor=color, facecolor='none')
...
def tracking_sink(boxes, t_boxes)
